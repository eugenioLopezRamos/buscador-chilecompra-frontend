...
class SearchesSaver extends React.Component {
    ...
    // handleResults = () => {
    //   //  let name = this.state.enteredResultName
    //     this.setState({showModal: {show: false}, enteredSearchName: "", enteredResultName: ""})
    //     this.props.handleResults(this.state.enteredResultName)        
    // }

    showModal = () => {
        this.setState({showModal: {show: true}});
    }
    ...
    render = () => {
        return (
            <div className="col-xs-12 no-gutter save-search-buttons">
                ...
                />
                <button type="button" className="btn btn-primary col-xs-6 col-md-4 col-md-offset-2 allow-wrap" onClick={() => {this.showModal()}}>Guardar parámetros de búsqueda (TBI)</button>
            </div>
            )
    }
