    ...
    const objectHandler = (object) => {
        ...
        return Object.keys(object).map((key, index) => {
            ...
            if(utils.isPrimitive(object[key])) {
                    if(!object[key]) {
                        return <div className="object-details-primitive" key={`object-details-primitive-${index}`}>{utils.pascalCaseToSentenceCase(key)}: (vacío)`</div>;
                    }
                    return <div className="object-details-primitive" key={`object-details-primitive-${index}`}>{utils.pascalCaseToSentenceCase(key)}: {object[key]}</div>;
            }

            return (<li className="object-details-subObject-title" key={`li-${index}`}>
                    {utils.pascalCaseToSentenceCase(key)}: <div key={`object-details-subobject-${index}`} className="object-details-subObject">{objectHandler(object[key])}</div>
                   </li>); 
        });
    };
