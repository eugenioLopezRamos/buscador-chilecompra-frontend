...
import * as actions from '../SearchQueryValuesActions';

describe('Tests search query values actions', () => {

    it('Should return a new value for the autofiller <input>', () => {
        const expectedAction = {type: types.AUTOFILLER_INPUT, value: "a new value"}
        expect(actions.autoFillerInput("a new value")).toEqual(expectedAction);
    });
    
    it('Should set a new value to set for startDate', () => {
        const mockDate = "2017-02-20T14:57:59.223Z";
        const expectedAction = {type: types.SET_SEARCH_START_DATE, value: mockDate}

        expect(actions.setStartDate(mockDate)).toEqual(expectedAction);
    });

    it('Should toggle alwaysFromToday from true to false', () => {
        let initialValue = true;
        const expectedAction = {type: types.TOGGLE_DATE_ALWAYS_FROM_TODAY, value: !initialValue};
        expect(actions.toggleDateAlwaysFromToday(initialValue)).toEqual(expectedAction);
    })

    it('Should toggle alwaysFromToday from false to true', () => {
